...
- `text`
  必填。自然语言指令本体，例如“给某人发微信说 hello”或“读取一下对方说了些什么”

- `--yes`
  可选。自动确认敏感动作，当前主要用于发送消息这类动作；不带时，某些动作会先停下来要求确认
...
- `--interval`
  可选。轮询间隔，单位秒，默认 `60`；表示每隔多久重新读取一次聊天内容
...
- `--cooldown`
  可选。自动回复冷却期，单位秒，默认 `180`
  如果我们刚自动发过消息，在冷却期内即使对方有新消息，也不会立刻再次自动回复

- `--context-rounds`
  可选。每轮带入多少轮最近历史上下文，默认 `3`
  数值越大，回复越能参考前文，但上下文也会更长

- `--persona-file`
  可选。微信主人的长期风格档案文件路径
  如果不传，默认尝试读取当前目录下的 `macagent-wechat-owner-profile.md`

- `--yes`
  可选。允许自动发送建议回复
  不带时只会观察、分析、记录，不会真的发消息

- `--log-file`
  可选。自定义 Markdown 日志路径
  不传时会自动在当前目录创建带时间戳的默认日志文件
